...
(function ($) {
    $.extend({
    	...
    	table: {
            ...
            init: function(options) {
                ...
                $('#' + options.id).bootstrapTable({
                    ...
                    contentType: $.common.isEmpty(options.contentType) ? contentType : options.contentType,   // 编码类型
                    method: 'post',                                     // 请求方式（*）
                    cache: false,                                       // 是否使用缓存
                    ...
                    pageList: [10, 25, 50,100,150,200,250,300,500,1000],// 可供选择的每页的行数（*）
                    ...
                    queryParams: $.common.isEmpty(options.contentType) ? options.queryParams : JSON.stringify(options.queryParams),                   // 传递参数（*）
            ...
            initEvent: function(data) {
            	...
            	$("#" + $.table._option.id).on("check.bs.table check-all.bs.table uncheck.bs.table uncheck-all.bs.table", function (e, rows) {
            		...
            		if ($.common.isNotEmpty($.table._option.rememberSelected) && $.table._option.rememberSelected) {
            			...
            			selectionIds = _[func](selectionIds, temp);
            		}
            	});
            },
            // 当所有数据被加载时触发
			...
			tooltip: function (value, length) {
				...
				return '<span class="tooltip-show" data-toggle="tooltip" title="' + _value + '">' + _text +'</span>';
			},
			// 下拉按钮切换
			...
			//项目报表查询
			searchOverView:function(formId,tableIdList){
				var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
				for (var i=0;i<tableIdList.length;i++){
					var params = $("#" + tableIdList[i]).bootstrapTable('getOptions');
					params.queryParams = function(params) {
						var search = {};
						$.each($("#" + currentId).serializeArray(), function(i, field) {
							search[field.name] = field.value;
					...
					$("#" + tableIdList[i]).bootstrapTable('refresh', params);
            ...
            // 导入数据
            importExcel: function(formId,importName) {
            	table.set();
            	var currentId = $.common.isEmpty(formId) ? 'importTpl' : formId;
            	var modalName = $.common.isEmpty(importName) ? table.options.modalName : importName;
            	layer.open({
            		type: 1,
            		area: ['400px', '230px'],
            		fix: false,
        ...
        treeTable: {
            ...
            init: function(options) {
                ...
                var treeTable = $('#' + options.id).bootstrapTreeTable({
					...
					clickRow:options.clickRow,                          //是否打开点击行选中-----hcl
					selectChildren:options.selectChildren                //是否打开点击选中所有子类-----hcl
        	    });
		...
		tree: {
			...
			hideDept:function(){
				...
				for (var i=0;i<nodes.length;i++){
					...
					$._tree.hideNodes(nodes[i].children);
				}
			},
			// 搜索节点
			searchNode: function() {
			...
			hideAllNode: function(nodes) {
				...
				tree.hideNodes(nodes[0].children);
			},
			// 显示所有父节点
			...
			// 不允许选择一二级目录
			notAllowParentsOne: function(_tree) {
				var nodes = _tree.getSelectedNodes();
				for (var i = 0; i < nodes.length; i++) {
					if (nodes[i].level == 0||nodes[i].level==1) {
						$.modal.msgError("不能选择一二级目录（" + nodes[i].name + "）");
						return false;
					}
				}
				return true;
    	...
    	modal: {
            ...
            openOptions: function (options) {
                ...
                layer.open({
                    ...
                    success: function(layero, index){
                        ...
                        body.find('#toolbar').addClass('hide');//得到iframe页的body内容
                        body.find('.modelHide').hide();
            ...
            // 弹出层全屏去掉按钮
            openFullS: function (title, url, width, height,flag) {
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
                    width = 'auto';
                    height = 'auto';
        ...
        operate: {
            ...
            detailTab:function(id){
        		console.log($.table._option);
            	 var _url = $.common.isEmpty(id) ? $.table._option.detailUrl : $.table._option.detailUrl.replace("{id}", id);
            	 $.modal.openTab("详情" + $.table._option.modalName, _url);
            },
            // 删除信息
            remove: function(id) {
			...
			// 修改信息，以tab页展现
			editTab2: function(projectId,id) {
			...
			editFullSAdd: function(id,updateUrl) {
				var url = "/404.html";
				if ($.common.isNotEmpty(id)) {
					url = updateUrl.replace("{id}", id);
				}
				$.modal.openFullS("查看详情", url);
			},
            // 修改访问地址
            editUrl: function(id) {
            	var url = "/404.html";
			...
			//保存json格式的信息，不刷新页面
			saveJsonSuccess: function(url,data){
				var config = {
					contentType : "application/json;charset=UTF-8",
					url: url,
					type: "post",
			...
			//弹框并刷新当前页面的table
			saveModalRefresh:function(url, data){
				var config = {
					url: url,
					type: "post",
					dataType: "json",
    	...
    	common: {
         ...
         // 获取obj对象长度
            ...
            uploadFile:function(option){           	
            	if($.common.isEmpty(option.uploadUrl)){
            		option.uploadUrl = ctx + 'common/bootstrapFileInputUpload';
            	}
            	if($.common.isEmpty(option.deleteUrl)){
            		option.deleteUrl = ctx+"common/deleteFile";
            	}
            	var btns = '<a class="kv-file-eye btn btn-sm btn-default" {dataKey} title="在线预览"><i class="fa fa-eye"></i></a> ';
            	btns += '<a  class="kv-file-down btn btn-sm btn-default" {dataKey} title="下载附件"><i class="fa fa-cloud-download"></i></a>';
            	...
            	$("#"+option.id).fileinput({
            		...
            		deleteExtraData : option.deleteExtraData,
            		otherActionButtons: btns,
            		overwriteInitial : false,
            		showDwn : true,
            		initialPreviewAsData : true,
            		initialPreview : option.imgsUrl,
            		initialPreviewConfig : option.downloadConfig,
            		showPreview : option.showPreview //是否显示预览，默认true
            	}).on("fileuploaded", function(event, data, previewId, index) {
            		if(option.attachmentList!=undefined){
            			attachmentList = option.attachmentList;
            		}
            		var obj = {};
            		obj.attachmentName = data.response.fileName;
            		obj.fileName = data.response.sourceFileName;
					...
					for(var i=0;i<attachmentList.length;i++){
						if(attachmentList[i].attachmentName==data.fileName){
							index=i
						}
					}
            	...
            	$(".kv-file-down").off('click').on('click',function(){
				...
				$(".kv-file-eye").off('click').on('click',function(){
            		var str = $(this).data('key');
            		var reg = new RegExp('/','g');
            		var newstr = str.replace(reg,'|');//由于转义字符发送请求会报错400，故先把/替换成|再去后台替换回来
            ...
            //回显字典
           dictV:function(option){//d:字典名 v:键值
    			$('.'+option.className).html($.table.selectDictLabel(option.dict, option.value));
    		},

			//上传文件到对象服务器功能
			uploadFileToObs:function(option){
