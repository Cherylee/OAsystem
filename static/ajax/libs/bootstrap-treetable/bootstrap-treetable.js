...
(function($) {
    ...
    $.fn.bootstrapTreeTable = function(options, param) {
        ...
        var renderTable = function(data) {
            ...
            if (rootNode) {
                $.each(rootNode, function(i, item) {
                    ...
                    recursionNode(item, 1, _child_row_id, "row_root","row_root");
                });
            }
        ...
        // 递归获取子节点并且设置子节点
        var recursionNode = function(parentNode, lv, row_id, p_id,row_id_p) {
            var $tbody = target.find("tbody");
            var _ls = target.data_list["_n_" + parentNode[options.code]];
            var row_id_p=row_id_p+' '+row_id;
            var $tr = renderRow(parentNode, _ls ? true : false, lv, row_id, p_id,row_id_p);
            $tbody.append($tr);
            if (_ls) {
                $.each(_ls, function(i, item) {
                    var _child_row_id = row_id + "_" + i
                    recursionNode(item, (lv + 1), _child_row_id, row_id,row_id_p)
                });
            }
        };
        // 绘制行
        var renderRow = function(item, isP, lv, row_id, p_id,row_id_p) {
            // 标记已显示
            item.isShow = true;
            item.row_id = row_id;
            item.p_id = p_id;
            item.lv = lv;
            var $tr = $('<tr id="' + row_id + '" pid="' + p_id + '" class="' + row_id_p + '"></tr>');
            ...
            target.find("tbody").find("tr").click(function() {
                ...
                if (target.hasSelectItem) {
                    ...
                    if (_ipt.attr("type") == "radio") {
                        ...
                        // }
                        //多选框样例---hcl
                    		if ($(this).hasClass('treetable-selected')) {

                        		_ipt.prop('checked', false);
                                $(this).removeClass("treetable-selected");

                                if($('.'+fid).length!=0){
                                   $('.'+fid).removeClass("treetable-selected");
                                   $('.'+fid).find("input[name='select_item']").prop('checked', false);
                                }


                        	}else if(!$(this).hasClass('treetable-selected')){

                        		_ipt.prop('checked', true);
                                $(this).addClass("treetable-selected");

                                if($('.'+fid).length!=0){
                                $('.'+fid).addClass("treetable-selected");
                                $('.'+fid).find("input[name='select_item']").prop('checked', true);
                                }
        ...
        //注册checkbox选中所有子类---hcl
        var clickParent=function(){
            target.find("tbody") .find("input[name='select_item']").click(function() {
                if($(this).attr("type")=='checkbox'){
                    var classParent=$(this).parent().parent().attr('id')
                    if($(this).prop('checked')){
                        $('.'+classParent).find("input[name='select_item']").prop('checked', true);
                    }else{
                        $('.'+classParent).find("input[name='select_item']").prop('checked', false);
                    }
                }
            })
        }
    ...
    $.fn.bootstrapTreeTable.defaults = {
        ...
        clickRow:false,       // 是否开启行点击
        selectChildren:false, // 是否开启点击选中所有子类
